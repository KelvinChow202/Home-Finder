...
export default function Offer() {
    ...
    useEffect(() => {
        async function fetchListings() {
            const q = query(collection(db, 'listings'),
                where('offer', '==', true), limit(4), orderBy('timestamp', 'desc'))
            try {
                const listings = []
    ...
    async function onFetchMoreListings() {
        setLoading(true)
        const q = query(collection(db, 'listings'),
            where('offer', '==', true), limit(4), orderBy('timestamp', 'desc'), startAfter(lastFectchedListing))
        try {
            const listings = []
            const querySnapshot = await getDocs(q)
    ...
    return (
        <div className='bg-pink-500 max-w-6xl mx-auto'>
